...
export class CathedralSQLiteEngine extends EventEmitter {
    ...
    `);
        const result = stmt.run(validated.id, validated.name, validated.type, validated.width, validated.height, validated.dpi, validated.color_space, validated.background_color, validated.canvas_settings, validated.created_at, validated.updated_at, validated.user_id, validated.collaboration_enabled, validated.auto_save_enabled, validated.auto_save_interval, validated.quality_standard);
     ...
     */
    async saveDesignElement(element) {
        const now = new Date().toISOString();
        const elementData = {
            ...element,
        ...
        const transaction = this.db.transaction((element) => {
            const stmt = this.db.prepare(`
      ...
      `);
            stmt.run(validated.id, validated.workspace_id, validated.type, validated.name, validated.position_x, validated.position_y, validated.width, validated.height, validated.transform_data, validated.style_data, validated.metadata, validated.created_at, validated.updated_at, validated.version, validated.layer_id, validated.parent_id, validated.z_index);
        });
    ...
    async performAutoSave(workspaceId) {
      ...
      `);
            stmt.run(saveId, workspaceId, new Date().toISOString(), elements.length, sizeBytes, duration, true, null);
      ...
      `);
            stmt.run(saveId, workspaceId, new Date().toISOString(), 0, 0, duration, false, error instanceof Error ? error.message : 'Unknown error');
    ...
    `);
        stmt.run(versionId, workspaceId, label, description, new Date().toISOString(), createdBy, null, // parent_version_id - would be set for branching
        elementsSnapshot, this.calculateQualityScore(elements), Buffer.byteLength(elementsSnapshot, 'utf8'));
     ...
     * Validate data integrity
     */
    validateDataIntegrity(elements) {
        try {
            ...
            for (const element of elements) {
                ...
                if (element.height !== null && element.height < 0)
                    return false;
                if (isNaN(element.position_x) || isNaN(element.position_y))
                    return false;
            }
            return true;
        }
     ...
     * Recovery from crash
     */
    async recoverFromCrash(workspaceId) {
        try {
        ...
        LIMIT 1
      `);
            const checkpoint = checkpointStmt.get(workspaceId);
            if (!checkpoint) {
            ...
            if (Array.isArray(checkpointData)) {
                ...
                for (const elementData of checkpointData) {
                    await this.saveDesignElement(elementData);
                }
                return {
                    recovered: true,
                    lastCheckpoint: checkpoint,
                    elements: checkpointData
                };
            }
            return { recovered: false };
        }
     ...
     */
    getAutoSaveHistory(workspaceId, limit = 10) {
        const stmt = this.db.prepare(`
     ...
     */
    cleanupOldAutoSaves(olderThanDays = 7) {
...
if (typeof window !== 'undefined') {
    window.cathedralSQLiteEngine = cathedralSQLiteEngine;
}
if (typeof globalThis !== 'undefined') {
    globalThis.cathedralSQLiteEngine = cathedralSQLiteEngine;
}
