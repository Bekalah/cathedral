...
export class TraumaSafeInterface {
    config;
    emergencyExitCallbacks = [];
    processingTimeouts = [];
    sensoryBreakTimer = null;
    isActive = true;
    constructor(config) {
    ...
    setupSensoryBreaks() {
        if (this.config.sensoryBreakInterval <= 0)
            return;
    ...
    // Public API methods
    registerEmergencyExit(callback) {
        this.emergencyExitCallbacks.push(callback);
    }
    async processWithDelay(operation) {
...
export class TraumaSafeCreativeTool {
    ...
    async safeCreativeOperation(operation, operationName) {
    ...
    async safeCharacterSelection(characterData, onSelect) {
    ...
    async safeGeometryGeneration(geometryParams, onGenerate) {
        await this.safeCreativeOperation(async () => {
...
export class AccessibilityManager {
    ...
    initializeVoiceCommands() {
        ...
        this.voiceCommands.set('safe exit', () => {
            const interface_ = document.querySelector('.trauma-safe-emergency-exit');
            if (interface_)
                interface_.click();
        });
