...
const lattice = latticeJson;
// Basic integrity check (non-throwing; detailed validation via validateCodex()).
const hasAnyNodes = Array.isArray(lattice.primary_nodes) && lattice.primary_nodes.length > 0 &&
    Array.isArray(lattice.gateway_nodes) && lattice.gateway_nodes.length > 0;
/**
 * Lookup helpers
 */
function findNode(id) {
    return (lattice.primary_nodes.find((n) => n.id === id) ||
 ...
 */
export function getCodexLattice() {
    return lattice;
}
...
export function resolveSymbol(symbol) {
    ...
    const geometryId = symbol.geometry_profile_id ?? node?.vector_profile_ids.geometry_profile_id;
    const colorId = symbol.color_profile_id ?? node?.vector_profile_ids.color_profile_id;
    const soundId = symbol.sound_profile_id ?? node?.vector_profile_ids.sound_profile_id;
    const crystalId = symbol.crystal_profile_id ?? node?.vector_profile_ids.crystal_profile_id;
    const archetypeId = symbol.archetype_profile_id ?? node?.vector_profile_ids.archetype_profile_id;
 ...
 */
export function samplePalette(nodeId) {
...
export function validateCodex() {
    ...
    if (all.length < 2) {
        issues.push({
            level: 'warning',
            message: 'Codex lattice contains only bootstrap nodes. Sync from external/codex-144-99 and PERSONAL_WORK_MANIFEST canonical datasets.',
...
export function generateReport() {
    ...
    lines.push(`Nodes: total=${report.summary.total_nodes}, primary=${report.summary.primary_nodes}, gateway=${report.summary.gateway_nodes}`);
    lines.push(`Profiles: geom=${report.summary.geometry_profiles}, color=${report.summary.color_profiles}, sound=${report.summary.sound_profiles}, crystal=${report.summary.crystal_profiles}, archetype=${report.summary.archetype_profiles}`);
    for (const issue of report.issues) {
        lines.push(`${issue.level.toUpperCase()}: ${issue.message}${issue.context ? ` ${JSON.stringify(issue.context)}` : ''}`);
    }
    return lines.join('\n');
}
