...
export class Codex144Engine {
    ...
    setupIntegrations() {
        ...
        this.activeConnections.set("godot", {
            ...
            protocol: "websocket",
            methods: {
                spawn_node: (nodeId) => this.spawnNodeInGodot(nodeId),
                fuse_consciousness: (fusionId) => this.processFusion(fusionId),
        ...
        this.activeConnections.set("web", {
            ...
            methods: {
                ...
                process_fusion: (nodeIds) => this.processFusionRequest(nodeIds),
        ...
        this.activeConnections.set("audio", {
            engine: "tonejs",
            methods: {
                play_node_frequency: (nodeId) => this.playNodeFrequency(nodeId),
                fuse_audio: (fusionId) => this.processAudioFusion(fusionId),
        ...
        this.activeConnections.set("fractal", {
            engine: "mandelbrot",
            methods: {
                generate_consciousness_fractal: (nodeId) => this.generateFractal(nodeId),
                fuse_fractals: (fusionId) => this.processFractalFusion(fusionId),
     ...
     */
    createCustomFusion(nodeIds) {
        const nodeResults = nodeIds.map(id => this.nodes.get(id)).filter((n) => n !== null);
    ...
    frequencyToNote(frequency) {
        ...
        const closest = noteMap.reduce((prev, curr) => Math.abs(curr.freq - frequency) < Math.abs(prev.freq - frequency) ? curr : prev);
        return closest.note;
    }
    getHarmonicSeries() {
...
if (typeof window !== 'undefined') {
    window.codex144Engine = codex144Engine;
}
if (typeof globalThis !== 'undefined') {
    globalThis.codex144Engine = codex144Engine;
}
export default codex144Engine;
//# sourceMappingURL=Codex144Engine.js.map