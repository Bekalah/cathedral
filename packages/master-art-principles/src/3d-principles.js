 ...
 */
export function goldenCamera(sceneSize, height = sceneSize) {
 ...
 */
export function masterLighting(sceneSize = 100) {
    return {
        ambient: {
            color: 0xffffff,
 ...
 */
export function createGoldenScene(renderer, width, height) {
    ...
    const camera = new THREE.PerspectiveCamera(cameraSettings.fov, width / height, cameraSettings.near, cameraSettings.far);
    camera.position.set(cameraSettings.position.x, cameraSettings.position.y, cameraSettings.position.z);
    camera.lookAt(cameraSettings.target.x, cameraSettings.target.y, cameraSettings.target.z);
    // Master lighting
 ...
 */
export function createSacredGeometryMesh(type, size = 10) {
    let points = [];
    switch (type) {
        case 'vesica':
 ...
 */
export function goldenEasing(t) {
 ...
 */
export function createFluidAnimation(duration = 2000, callback) {
...
//# sourceMappingURL=3d-principles.js.map