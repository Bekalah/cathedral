...
// Load export profiles from REGISTRY data
const loadExportProfiles = async (): Promise<ExportProfile[]> => {
  try {
    console.log('📦 Loading REGISTRY-driven export profiles...');
    
    // Load REGISTRY data for export profiles
    const profiles: ExportProfile[] = [
      {
        id: 'professional_web',
        name: 'Professional Web Export',
        format: 'svg',
        quality: 'high',
        settings: {
          width: 1920,
        ...
        features: ['vector_optimization', 'color_management', 'responsive_scaling'],
        registry_source: 'REGISTRY/styles/style_packs.vertical_slice.json'
      },
      {
        id: 'print_professional',
        ...
        features: ['print_optimization', 'color_profile_management', 'bleed_marks'],
        registry_source: 'REGISTRY/palettes/palette_catalog.vertical_slice.json'
      },
      {
        id: 'social_media',
        ...
        features: ['social_optimization', 'compression_optimization', 'metadata_inclusion'],
        registry_source: 'REGISTRY/rooms/room_catalog.vertical_slice.json'
      }
    ];

    console.log(`✅ Loaded ${profiles.length} REGISTRY-driven export profiles`);
    return profiles;
    
  } catch (error) {
    console.error('❌ Failed to load export profiles from REGISTRY:', error);
    // Return fallback profiles
    return [
      {
        id: 'default_web',
        name: 'Default Web Export',
        format: 'svg',
        quality: 'standard',
        settings: {
          width: 1920,
